...
def downloadSaveData():
    ...
    # Convert images to JPEG and save to disk
...
def split_data(input_dir, output_dir, n_clients):
    ...
    for label in range(10):
        ...
        for i in range(n_clients):
            # Calculate the start and end index of the files for this partition
            start_index = i * num_files_per_partition
            end_index = start_index + num_files_per_partition
            
            # If this is the last partition, include any remaining files
            if i == n_clients - 1:
                end_index = len(file_names)
            
            # Create a new directory for this client if there is not one
            if label == 0:
                client_dir = os.path.join(output_dir, f'client_{i}')
                os.makedirs(client_dir, exist_ok=True)

            # Create a new directory for this partition
            client_dir_label = os.path.join(output_dir, f'client_{i}/{label}')
            os.makedirs(client_dir_label, exist_ok=True)
            
            # Copy the files for this partition into the new directory
...
if __name__ == '__main__':
    ...
    downloadSaveData()
    split_data("mnist_data/images", "mnist_data", n_clients)